...
public class NewColorPickerView extends View {
    ...
    public NewColorPickerView(Context context) {
        this(context, null, 0);
    }

    public NewColorPickerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NewColorPickerView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs, R.styleable.NewColorPickerView, defStyle, defStyle);

        try {
            mDataThickness = a.getDimension(
                    R.styleable.NewColorPickerView_dataThickness3, mDataThickness);
            mDataColor = a.getColor(
                    R.styleable.NewColorPickerView_dataColor3, mDataColor);
        } finally {
            a.recycle();
        }
		// Get the current hue from the current color and update the main color field
		float[] hsv = new float[3];
		Color.colorToHSV(mDataColor, hsv);
		mCurrentHue = hsv[0];
		updateMainColors();

		mCurrentColor = mDataColor;
